// backend/services/svcconfig/src/controllers/svcconfig.update.controller/handlers/bagToDb.update.handler.ts
/**
 * Docs:
...
export class BagToDbUpdateHandler extends HandlerBase {
  ...
  protected async execute(): Promise<void> {
    ...
    // --- Required context ----------------------------------------------------
    const bag = this.ctx.get<DtoBag<SvcconfigDto>>("bag");
    if (!bag) {
    ...
    // --- Writer (bag-centric) -----------------------------------------------
    const writer = new DbWriter<SvcconfigDto>({ bag, svcEnv });
