// backend/services/user/src/controllers/user.update.controller/handlers/applyPatch.update.handler.ts
/**
 * Docs:
 ...
 *   payload (from ctx["bag"]) — both are **singleton DtoBags<UserDto>**.
 * - Output a **singleton DtoBag<UserDto>** with the UPDATED DTO and replace ctx["bag"] with it.
 *
 * Inputs (ctx):
 * - "existingBag": DtoBag<UserDto>   (singleton; from LoadExistingUpdateHandler)
 * - "bag": DtoBag<UserDto>           (singleton; from BagPopulateGetHandler — the patch)
 *
 * Outputs (ctx):
 * - "bag": DtoBag<UserDto>           (REPLACED with updated singleton bag)
...
export class ApplyPatchUpdateHandler extends HandlerBase {
  ...
  protected async execute(): Promise<void> {
    // ---- Fetch typed bags ---------------------------------------------------
    const existingBag = this.ctx.get<DtoBag<UserDto>>("existingBag");
    const patchBag = this.ctx.get<DtoBag<UserDto>>("bag");
    ...
    if (!(existing instanceof UserDto) || !(patchDto instanceof UserDto)) {
      this.ctx.set("handlerStatus", "error");
      this.ctx.set("status", 400);
      this.ctx.set("error", {
        code: "TYPE_MISMATCH",
        title: "Bad Request",
        detail:
          "DtoBag type mismatch: expected UserDto for both existing and patch items.",
      });
      return;
    }
    ...
    try {
      const dtoType = this.ctx.get<string>("dtoType"); // "user" on this route
      if (
        dtoType &&
