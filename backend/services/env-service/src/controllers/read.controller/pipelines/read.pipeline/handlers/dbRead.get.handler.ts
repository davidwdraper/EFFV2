// backend/services/env-service/src/controllers/read.controller/pipelines/read.pipeline/handlers/dbRead.get.handler.ts
/**
 * Docs:
 ...
 *
 * Invariants:
 * - Canonical id is the DTO's `id` property; URL param is `:id`.
 * - Self-contained: constructs its own DbReader.
 * - svcEnv is obtained from ControllerBase via HandlerBase (no ctx plumbing).
 * - dtoCtor must be seeded by the pipeline via ctx.set("read.dtoCtor", EnvServiceDto).
 */
