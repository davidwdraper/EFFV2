...
/**
 * Standard loader for all services.
 * Order (missing ignored; last wins):
 *   1) <repo>/.env       (discovered by walking up from service root)
 *   2) <repo>/.env.dev   (walk up)
 *   3) <service>/.env
 *   4) <service>/.env.dev
 *   5) ENV_FILE (absolute or CWD-relative) — explicit override, loaded last
 *
 * Returns both the candidate list ("tried") and those that actually existed ("present"),
 * so callers can log what happened.
 */
export function loadStandardEnvOrThrow(
  serviceRootAbs: string,
  overrideEnvFile?: string | null
): { tried: string[]; present: string[] } {
  const repoEnv = findUp(serviceRootAbs, [".env"]);
  const repoEnvDev = findUp(serviceRootAbs, [".env.dev"]);

  const svcEnv = path.join(serviceRootAbs, ".env");
  const svcEnvDev = path.join(serviceRootAbs, ".env.dev");

  const override = overrideEnvFile
    ? path.isAbsolute(overrideEnvFile)
      ? overrideEnvFile
      : path.resolve(process.cwd(), overrideEnvFile)
    : null;

  const tried = [repoEnv, repoEnvDev, svcEnv, svcEnvDev, override].filter(
    Boolean
  ) as string[];
  const present = tried.filter((f) => fs.existsSync(f));

  // If an explicit override was provided but missing, fail loudly.
  if (overrideEnvFile && !override) {
    throw new Error(`ENV_FILE is invalid: "${overrideEnvFile}"`);
  }
  if (override && !fs.existsSync(override)) {
    throw new Error(`ENV_FILE not found: ${override}`);
  }

  loadEnvFilesOrThrow(present, { allowMissing: true, order: "last-wins" });
  return { tried, present };
}
