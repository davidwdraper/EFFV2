...
export class SomeHandlerHappyTest extends HandlerTestBase {
  ...
  protected async execute(): Promise<void> {
    const ctx = this.makeCtx({
      ...
      op: "some.op",
      body: { /* handler‑specific test payload */ },
    });
...
export class ToBagUserHappyTest extends HandlerTestBase {
  ...
  protected async execute(): Promise<void> {
    ...
    this.assertEq(String(handlerStatus ?? ""), "ok", "handlerStatus should be 'ok' on happy path");
    ...
    this.assertEq(String(items.length), "1", "DtoBag should contain exactly one UserDto");

    const userDto: any = items[0];
    const dtoId =
      userDto && typeof userDto.getId === "function" ? userDto.getId() : undefined;
...
export class ToBagUserMissingUserIdTest extends HandlerTestBase {
  ...
  protected async execute(): Promise<void> {
    ...
    const statusCode =
      rawResponseStatus !== undefined ? rawResponseStatus : ctx.get<number>("status");
...
export async function getScenarios() {
  return [
    ...
    },
    {
      ...
      name:
        "auth.signup: ToBagUserHandler fails with 500 when signup.userId is missing",
      shortCircuitOnFail: true,
...
This is the **canonical multi‑scenario pattern**.
...
Follow this guide and each new handler test should be a slam dunk—no runner debugging, just scenario logic and assertions.
