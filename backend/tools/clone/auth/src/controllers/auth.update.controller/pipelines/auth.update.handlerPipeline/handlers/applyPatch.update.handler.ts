// backend/services/auth/src/controllers/auth.update.controller/handlers/applyPatch.update.handler.ts
/**
 * Docs:
 ...
 *   payload (from ctx["bag"]) — both are **singleton DtoBags<AuthDto>**.
 * - Output a **singleton DtoBag<AuthDto>** with the UPDATED DTO and replace ctx["bag"] with it.
 *
 * Inputs (ctx):
 * - "existingBag": DtoBag<AuthDto>   (singleton; from LoadExistingUpdateHandler)
 * - "bag": DtoBag<AuthDto>           (singleton; from BagPopulateGetHandler — the patch)
 *
 * Outputs (ctx):
 * - "bag": DtoBag<AuthDto>           (REPLACED with updated singleton bag)
...
export class ApplyPatchUpdateHandler extends HandlerBase {
  ...
  protected async execute(): Promise<void> {
    // ---- Fetch typed bags ---------------------------------------------------
    const existingBag = this.ctx.get<DtoBag<AuthDto>>("existingBag");
    const patchBag = this.ctx.get<DtoBag<AuthDto>>("bag");
    ...
    if (!(existing instanceof AuthDto) || !(patchDto instanceof AuthDto)) {
      this.ctx.set("handlerStatus", "error");
      this.ctx.set("status", 400);
      this.ctx.set("error", {
        code: "TYPE_MISMATCH",
        title: "Bad Request",
        detail:
          "DtoBag type mismatch: expected AuthDto for both existing and patch items.",
      });
      return;
    }
    ...
    try {
      const dtoType = this.ctx.get<string>("dtoType"); // "auth" on this route
      if (
        dtoType &&
