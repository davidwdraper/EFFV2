// backend/services/handler-test/src/controllers/handler-test.update.controller/handler-test.update.controller.ts
/**
 * Docs:
 ...
 *
 * Purpose:
 * - Orchestrate PATCH /api/handler-test/v1/:dtoType/update/:id
 * - Thin controller: choose per-dtoType pipeline; pipeline defines handler order.
 *
 * Invariants:
 * - Canonical path param is "id" (legacy :handlerTestId normalized to ctx["id"]).
...
import * as HandlerTestUpdatePipeline from "./pipelines/update.handlerPipeline";
...
export class HandlerTestUpdateController extends ControllerJsonBase {
  ...
  public async patch(req: Request, res: Response): Promise<void> {
    ...
    // Normalize param to canonical "id" (stop handlerTestId drift)
    const idParam =
      (req.params as any)?.id ?? (req.params as any)?.handlerTestId ?? null;
    ctx.set("id", idParam);
    ...
    switch (dtoType) {
      case "handler-test": {
        this.seedHydrator(ctx, "handler-test", { validate: true });
        const steps = HandlerTestUpdatePipeline.getSteps(ctx, this);
