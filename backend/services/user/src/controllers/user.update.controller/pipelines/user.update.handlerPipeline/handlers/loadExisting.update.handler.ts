// backend/services/user/src/controllers/user.update.controller/handlers/loadExisting.update.handler.ts
/**
 * Docs:
 ...
 *
 * Purpose:
 * - Build DbReader<UserDto> and load existing doc by canonical ctx["id"].
 * - Returns a **DtoBag** (0..1) as ctx["existingBag"] (does NOT overwrite ctx["bag"]).
 *
 * Inputs (ctx):
 * - "id": string (required; controller sets from :id or :userId)
 * - "update.dtoCtor": DTO class (required)
 *
 * Outputs (ctx):
 * - "existingBag": DtoBag<UserDto>  (size 0 or 1)
 * - "dbReader": DbReader<UserDto>
 */
  ...
  protected async execute(): Promise<void> {
    ...
    if (!id) {
      ...
      this.ctx.set("error", {
        ...
        hint: "PATCH /api/user/v1/<id> with JSON body of fields to update.",
      });
      this.log.debug(
