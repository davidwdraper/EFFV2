 ...
 *
 * Purpose:
 * - Concrete DTO for the template service ("auth").
...
export class AuthDto extends BaseDto implements IDto {
  // ← implements IDto
  ...
  public static fromJson(json: unknown, opts?: { validate?: boolean }): AuthDto {
    const dto = new AuthDto(BaseDto.getSecret());
  ...
  public toJson(): AuthJson {
    ...
    const body = {
      ...
      type: "auth" as const, // wire type (matches slug)
  ...
  // inside backend/services/shared/src/dto/templates/auth/auth.dto.ts
  ...
  public clone(newId?: string): this {
    ...
    const Ctor = this.constructor as typeof AuthDto;
