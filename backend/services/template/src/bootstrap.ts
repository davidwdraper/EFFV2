...
function resolvePaths() {
  ...
  const serviceDir = path.resolve(__dirname, ".."); // .../backend/services/template/src -> /template
  const svcRoot = path.resolve(serviceDir, ".."); // .../backend/services/template
...
(function bootstrap() {
  ...
  // ✅ Template-only required keys (no ACT_* here)
  assertRequiredEnv([
    "LOG_LEVEL",
    "LOG_SERVICE_URL",
    "TEMPLATE_PORT",
