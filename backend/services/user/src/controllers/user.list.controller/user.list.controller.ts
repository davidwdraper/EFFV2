// backend/services/user/src/controllers/user.list.controller/user.list.controller.ts
/**
 * Docs:
 ...
 *
 * Purpose:
 * - Orchestrate GET /api/user/v1/:dtoType/list
...
import * as UserListPipeline from "./pipelines/user.list.handlerPipeline";
...
export class UserListController extends ControllerJsonBase {
  ...
  public async get(req: Request, res: Response): Promise<void> {
    ...
    switch (dtoType) {
      case "user": {
        this.seedHydrator(ctx, "user", { validate: false }); // <== don't need to validate
        const steps = UserListPipeline.getSteps(ctx, this);
