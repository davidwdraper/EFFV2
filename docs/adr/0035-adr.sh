...
# 0035-adr.sh — one-shot generator for ADR-0035
# Run this script from your repo root to create docs/adr/0035-ports-and-adapters-s2s-and-edge-jwt.md
#
# Docs:
...
#   - docs/adr/0034-centralized-service-discovery-via-gateway.md

${HOME}/eff/docs/docScripts/adrdoc.sh \
  "Ports & Adapters Stabilization for S2S Resolution and Edge JWT" \
  --status "Proposed" \
  --deciders "David Draper" \
  --tags "architecture, svcconfig, gateway, s2s, jwt, jose, modularity, ports, adapters" \
  --id 0035 \
  --context "A recent refactor moved the source of service-to-service (S2S) URLs and discovery logic from per-service environment variables to a centralized svcconfig mirror. Despite being a seemingly localized change, it cascaded across dozens of files and broke previously passing smoke tests.
...
These factors violated the intended microservice isolation, making small design changes ripple through the entire backend." \
  --decision "Introduce explicit **ports** and **adapters** to isolate cross-cutting concerns and stop import-time side effects:
...
- Centralized ServiceDirectory ensures audit logs and metrics can track all svcconfig-based resolutions." \
  --alternatives "1) Keep current shared utils with side effects — Rejected. Maintains tight coupling and import-time fragility.
...
4) Push S2S discovery back to per-service env vars — Rejected. Returns to duplicated config and higher operational risk." \
  --references "- SOP: docs/architecture/backend/SOP.md
...
echo "ADR 0035 generation invoked. Check docs/adr/0035-ports-and-adapters-s2s-and-edge-jwt.md"
