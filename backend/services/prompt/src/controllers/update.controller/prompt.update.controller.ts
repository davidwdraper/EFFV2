// backend/services/prompt/src/controllers/prompt.update.controller/prompt.update.controller.ts
/**
 * Docs:
 ...
 *
 * Purpose:
 * - Orchestrate PATCH /api/prompt/v1/:dtoType/update/:id
 * - Thin controller: choose per-dtoType pipeline; pipeline defines handler order.
 *
 * Invariants:
 * - Canonical path param is "id" (legacy :promptId normalized to ctx["id"]).
...
import * as PromptUpdatePipeline from "./pipelines/prompt.update.handlerPipeline";
...
export class PromptUpdateController extends ControllerJsonBase {
  ...
  public async patch(req: Request, res: Response): Promise<void> {
    ...
    // Normalize param to canonical "id" (stop promptId drift)
    const idParam =
      (req.params as any)?.id ?? (req.params as any)?.promptId ?? null;
    ctx.set("id", idParam);
    ...
    switch (dtoType) {
      case "prompt": {
        ...
        const steps = PromptUpdatePipeline.getSteps(ctx, this);
