// backend/services/auth/src/controllers/auth.update.controller/auth.update.controller.ts
/**
 * Docs:
 ...
 *
 * Purpose:
 * - Orchestrate PATCH /api/auth/v1/:dtoType/update/:id
 * - Thin controller: choose per-dtoType pipeline; pipeline defines handler order.
 *
 * Invariants:
 * - Canonical path param is "id" (legacy :authId normalized to ctx["id"]).
...
import * as AuthUpdatePipeline from "./pipelines/auth.update.handlerPipeline";
...
export class AuthUpdateController extends ControllerBase {
  ...
  public async patch(req: Request, res: Response): Promise<void> {
    ...
    // Normalize param to canonical "id" (stop authId drift)
    const idParam =
      (req.params as any)?.id ?? (req.params as any)?.authId ?? null;
    ctx.set("id", idParam);
    ...
    switch (dtoType) {
      case "auth": {
        ...
        const steps = AuthUpdatePipeline.getSteps(ctx, this);
