 ...
 *
 * Purpose:
 * - Orchestrate PATCH /api/env-service/v1/:envServiceId
...
export class EnvServiceUpdateController extends ControllerBase {
  ...
  public async patch(req: Request, res: Response): Promise<void> {
    ...
    const handlers = [
      new LoadExistingUpdateHandler(ctx), // reads by :envServiceId → ctx["existing"]
      new ApplyPatchUpdateHandler(ctx), // merges req.body → EnvServiceDto → ctx["updated"]
