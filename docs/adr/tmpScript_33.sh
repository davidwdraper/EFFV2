...
# 0033-adr.sh — one-shot generator for ADR-0033
# Run this script from your repo root to create docs/adr/0033-*.md
#
# Docs:
...
- Produces clear, auditable validation of required configuration." \
  --decision "Adopt a structured initialization model with deterministic environment discovery and deferred configuration reads:
...
- Secrets are never logged; validation output redacts values while naming missing keys." \
  --alternatives "1) Keep relying on the runner to source env — Rejected.
...
echo "ADR 0033 generation invoked. Check docs/adr/0033-*.md"
