// backend/services/svcconfig/src/controllers/svcconfig.update.controller/handlers/applyPatch.update.handler.ts
/**
 * Docs:
...
export class ApplyPatchUpdateHandler extends HandlerBase {
  ...
  protected async execute(): Promise<void> {
    // ---- Fetch typed bags ---------------------------------------------------
    const existingBag = this.ctx.get<DtoBag<SvcconfigDto>>("existingBag");
    const patchBag = this.ctx.get<DtoBag<SvcconfigDto>>("bag");
    ...
    if (!(existing instanceof SvcconfigDto) || !(patchDto instanceof SvcconfigDto)) {
      this.ctx.set("handlerStatus", "error");
      this.ctx.set("status", 400);
      this.ctx.set("error", {
        code: "TYPE_MISMATCH",
    ...
    try {
      const dtoType = this.ctx.get<string>("dtoType"); // "svcconfig" on this route
      if (
        dtoType &&
