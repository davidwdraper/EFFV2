// backend/services/test-log/src/app.ts
/**
 * Docs:
 ...
 * - For test-log, DB/index ensure is ON (checkDb=true).
 */
...
import { buildTestLogRouter } from "./routes/test-log.route";
...
class test-logApp extends AppBase {
  ...
  constructor(opts: CreateAppOptions) {
    ...
    super({
      ...
      // test-log is DB-backed: requires NV_MONGO_* and index ensure at boot.
      checkDb: true,
    });
  ...
  protected override mountRoutes(): void {
    ...
    const r: Router = buildTestLogRouter(this);
    this.app.use(base, r);
...
export default async function createApp(
  ...
  const app = new test-logApp(opts);
