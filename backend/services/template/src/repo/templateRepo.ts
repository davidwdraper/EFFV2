...
export async function list(opts: { limit?: number; offset?: number } = {}): Promise<TemplateDomain[]> {
  const { limit = 50, offset = 0 } = opts;
  const docs = await TemplateModel.find({}).skip(offset).limit(limit).exec();
  return docs.map((d: any) => dbToDomain(d));
}

export async function update(id: string, patch: Partial<TemplateDomain>): Promise<TemplateDomain | null> {
  const now = new Date();
  const updateDoc: any = { ...patch, dateLastUpdated: now };
  delete updateDoc._id;
  delete updateDoc.dateCreated;

  const doc = await TemplateModel.findByIdAndUpdate(id, updateDoc, { new: true }).exec();
  return doc ? dbToDomain(doc as any) : null;
}
