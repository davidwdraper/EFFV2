...
# 0036-adr.sh — one-shot generator for ADR-0036
# Run this script from your repo root to create docs/adr/0036-gateway-first-e2e-smoke-testing.md
#
# Docs:
...
- Multiple interfaces under test increase fragility and confusion." \
  --decision "Adopt a **Gateway-First E2E** testing model for all smoke tests except health and explicitly anonymous scenarios.
...
- Logs correlate reqId across gateway and services for reliable triage." \
  --alternatives "1) Keep mixed direct-service + gateway tests — Rejected. Misses edge regressions and causes drift.
...
echo "ADR 0036 generation invoked. Check docs/adr/0036-gateway-first-e2e-smoke-testing.md"
