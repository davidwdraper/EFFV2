 ...
 *
 * Purpose:
 * - Delete a single document by id for DELETE /delete/:envServiceId
...
export class DbDeleteDeleteHandler extends HandlerBase {
  ...
  protected async execute(): Promise<void> {
    ...
    const idRaw = typeof params.envServiceId === "string" ? params.envServiceId.trim() : "";
    if (!idRaw) {
      this.ctx.set("handlerStatus", "error");
      this.ctx.set("status", 400);
      this.ctx.set("error", {
        code: "BAD_REQUEST",
        message: "Missing required path parameter ':envServiceId'.",
        hint: "Call DELETE /api/env-service/v1/delete/<_id>. If your _id is a Mongo ObjectId, pass the 24-hex value.",
      });
      this.log.warn(
