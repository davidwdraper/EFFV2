// backend/services/gateway/src/controllers/gateway.update.controller/gateway.update.controller.ts
/**
 * Docs:
 ...
 *
 * Purpose:
 * - Orchestrate PATCH /api/gateway/v1/:dtoType/update/:id
 * - Thin controller: choose per-dtoType pipeline; pipeline defines handler order.
 *
 * Invariants:
 * - Canonical path param is "id" (legacy :gatewayId normalized to ctx["id"]).
...
import * as GatewayUpdatePipeline from "./pipelines/update.handlerPipeline";
...
export class GatewayUpdateController extends ControllerJsonBase {
  ...
  public async patch(req: Request, res: Response): Promise<void> {
    ...
    // Normalize param to canonical "id" (stop gatewayId drift)
    const idParam =
      (req.params as any)?.id ?? (req.params as any)?.gatewayId ?? null;
    ctx.set("id", idParam);
    ...
    switch (dtoType) {
      case "gateway": {
        this.seedHydrator(ctx, "gateway", { validate: true });
        const steps = GatewayUpdatePipeline.getSteps(ctx, this);
