// backend/services/env-service/src/controllers/env-service.delete.controller/handlers/dbDelete.delete.handler.ts
/**
 * Docs:
 ...
 *
 * Purpose:
 * - Delete a single document by DTO id for DELETE /delete/:envServiceId
 ...
 *
 * Invariants:
 * - Handlers speak DTO-space ids (envServiceId:string). Mongo details are hidden behind adapter helpers.
 */
  ...
  protected async execute(): Promise<void> {
    ...
    if (!svcEnv || !dtoCtor) {
      ...
      this.ctx.set("error", {
        ...
        detail:
          "Required context missing (svcEnv or dtoCtor). Ops: verify ControllerBase.makeContext() and EnvServiceDeleteController seeding.",
      });
      this.log.error(
    ...
    if (!idRaw) {
      ...
      this.ctx.set("error", {
        code: "BAD_REQUEST",
        title: "Bad Request",
        detail: "Missing required path parameter ':envServiceId'.",
        hint: "Call DELETE /api/env-service/v1/delete/<envServiceId>.",
      });
      this.log.warn(
        { event: "bad_request", reason: "no_id" },
        "Missing :envServiceId"
      );
      return;
    }
