...
# 0034-adr.sh — one-shot generator for ADR-0034
# Run this script from your repo root to create docs/adr/0034-*.md
#
# Docs:
...
- Keep migration simple for run.sh, smoke tests, and CI." \
  --decision "Adopt gateway-centered discovery and dual-port topology:
...
- Blast radius reduction: svcconfig outages primarily impact gateway; services degrade predictably." \
  --alternatives "1) Per-service SVCCONFIG_BASE_URL — Rejected.
...
echo "ADR 0034 generation invoked. Check docs/adr/0034-*.md"
