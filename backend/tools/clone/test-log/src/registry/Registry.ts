...
export class Registry extends ServiceRegistryBase {
   ...
   * Keys are the stable wire/type identifiers (e.g., "test-log").
   */
  protected ctorByType(): Record<string, DtoCtor<IDto>> {
    return {
      ["test-log"]: TestRunDto as unknown as DtoCtor<IDto>,
   ...
   * to a DTO instance.
   *
   * The test-log template does not apply field-level security by default,
  ...
  /** Hydrate an TestRunDto from JSON (validates if requested) and seed collection. */
  public fromJsonTestLog(
    json: unknown,
