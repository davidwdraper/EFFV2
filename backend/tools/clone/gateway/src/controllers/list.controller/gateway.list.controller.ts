// backend/services/gateway/src/controllers/gateway.list.controller/gateway.list.controller.ts
/**
 * Docs:
 ...
 *
 * Purpose:
 * - Orchestrate GET /api/gateway/v1/:dtoType/list
...
import * as GatewayListPipeline from "./pipelines/list.handlerPipeline";
...
export class GatewayListController extends ControllerJsonBase {
  ...
  public async get(req: Request, res: Response): Promise<void> {
    ...
    switch (dtoType) {
      case "gateway": {
        this.seedHydrator(ctx, "gateway", { validate: false }); // <== don't need to validate
        const steps = GatewayListPipeline.getSteps(ctx, this);
