 ...
 *
 * Purpose (template):
 * - Pure orchestration entrypoint for a CRUD-style user-auth service.
...
const LOG_FILE = path.resolve(process.cwd(), "user-auth-startup-error.log");
...
(async () => {
  try {
    ...
    const { app } = await createApp({
      slug: SERVICE_SLUG,
      version: SERVICE_VERSION,
      envLabel, // <-- this is what user-auth/app.ts expects
      envDto: primary,
