// backend/services/user-auth/src/controllers/user-auth.update.controller/handlers/applyPatch.update.handler.ts
/**
 * Docs:
 ...
 *   payload (from ctx["bag"]) — both are **singleton DtoBags<UserAuthDto>**.
 * - Output a **singleton DtoBag<UserAuthDto>** with the UPDATED DTO and replace ctx["bag"] with it.
 *
 * Inputs (ctx):
 * - "existingBag": DtoBag<UserAuthDto>   (singleton; from LoadExistingUpdateHandler)
 * - "bag": DtoBag<UserAuthDto>           (singleton; from BagPopulateGetHandler — the patch)
 *
 * Outputs (ctx):
 * - "bag": DtoBag<UserAuthDto>           (REPLACED with updated singleton bag)
...
export class ApplyPatchUpdateHandler extends HandlerBase {
  ...
  protected async execute(): Promise<void> {
    // ---- Fetch typed bags ---------------------------------------------------
    const existingBag = this.ctx.get<DtoBag<UserAuthDto>>("existingBag");
    const patchBag = this.ctx.get<DtoBag<UserAuthDto>>("bag");
    ...
    if (!(existing instanceof UserAuthDto) || !(patchDto instanceof UserAuthDto)) {
      this.ctx.set("handlerStatus", "error");
      this.ctx.set("status", 400);
      this.ctx.set("error", {
        code: "TYPE_MISMATCH",
        title: "Bad Request",
        detail:
          "DtoBag type mismatch: expected UserAuthDto for both existing and patch items.",
      });
      return;
    }
    ...
    try {
      const dtoType = this.ctx.get<string>("dtoType"); // "user-auth" on this route
      if (
        dtoType &&
