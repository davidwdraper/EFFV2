// backend/services/auth/src/controllers/auth.list.controller/auth.list.controller.ts
/**
 * Docs:
 ...
 *
 * Purpose:
 * - Orchestrate GET /api/auth/v1/:dtoType/list
...
import * as AuthListPipeline from "./pipelines/auth.list.handlerPipeline";
...
export class AuthListController extends ControllerBase {
  ...
  public async get(req: Request, res: Response): Promise<void> {
    ...
    switch (dtoType) {
      case "auth": {
        this.seedHydrator(ctx, "auth", { validate: false }); // <== don't need to validate
        const steps = AuthListPipeline.getSteps(ctx, this);
