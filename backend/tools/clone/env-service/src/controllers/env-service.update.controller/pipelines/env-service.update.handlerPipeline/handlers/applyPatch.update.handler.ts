...
export class ApplyPatchUpdateHandler extends HandlerBase {
  ...
  protected async execute(): Promise<void> {
    // ---- Fetch typed bags ---------------------------------------------------
    const existingBag = this.ctx.get<DtoBag<EnvServiceDto>>("existingBag");
    const patchBag = this.ctx.get<DtoBag<EnvServiceDto>>("bag");
    ...
    if (!(existing instanceof EnvServiceDto) || !(patchDto instanceof EnvServiceDto)) {
      this.ctx.set("handlerStatus", "error");
      this.ctx.set("status", 400);
      this.ctx.set("error", {
        code: "TYPE_MISMATCH",
