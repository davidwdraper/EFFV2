...
# 0032-adr.sh — one-shot generator for ADR-0032
# Run this script from your repo root to create docs/adr/0032-*.md

$HOME/eff/docs/docScripts/adrdoc.sh \
  "Route Policy via svcconfig + E2E Context Token and Ephemeral Per-Hop S2S" \
  --status "Proposed" \
  --deciders "David Draper" \
  --tags "security, auth, svcconfig, gateway, s2s, kms, tokens, policy" \
  --id 0032 \
  --context "We’re building a ground-up backend where the edge gateway is the sole public entry point, and internal worker services are called via a shared callBySlug() function. We recently switched to Google KMS and public/private keys for minting service-to-service (S2S) JWTs.
...
- No leaking client JWTs downstream of the edge." \
  --decision "Adopt a versioned Route Access Policy in svcconfig (default-deny) and enforce it at the gateway with optional re-enforcement at workers. Introduce a two-token model to preserve strict security without per-hop KMS cost:
...
- Introduce observe-only mode for policy enforcement to de-risk rollout." \
  --alternatives "1) Reuse the first S2S token across downstream hops — Rejected.
