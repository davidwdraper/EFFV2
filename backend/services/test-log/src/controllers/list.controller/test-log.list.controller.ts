// backend/services/test-log/src/controllers/test-log.list.controller/test-log.list.controller.ts
/**
 * Docs:
 ...
 *
 * Purpose:
 * - Orchestrate GET /api/test-log/v1/:dtoType/list
...
import * as TestLogListPipeline from "./pipelines/list.handlerPipeline";
...
export class TestLogListController extends ControllerJsonBase {
  ...
  public async get(req: Request, res: Response): Promise<void> {
    ...
    switch (dtoType) {
      case "test-run":
      case "test-handler": {
        this.seedHydrator(ctx, "test-log", { validate: false }); // <== don't need to validate
        const steps = TestLogListPipeline.getSteps(ctx, this);
