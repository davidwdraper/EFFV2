...
import SvcServiceModel from "../../../models/svcconfig.model";
import { dbToDomain } from "../../../mappers/svcconfig.mapper";

export async function list(req: Request, res: Response, next: NextFunction) {
  const requestId = String(req.headers["x-request-id"] || "");
  logger.debug({ requestId }, "[SrcServiceHandlers.list] enter");
  try {
    ...
    const docs = await SvcServiceModel.find({}).skip(skip).limit(limit);
    const items = docs.map((d) => dbToDomain(d as any));

    logger.debug(
      { requestId, count: items.length },
      "[SrcServiceHandlers.list] exit"
    );
    res.json({ items, count: items.length, limit, skip });
  } catch (err) {
    logger.debug({ requestId, err }, "[SrcServiceHandlers.list] error");
    next(err);
  }
}
