// backend/services/test-log/src/index.ts
/**
 * Docs:
 ...
 *
 * Purpose (template):
 * - Pure orchestration entrypoint for a CRUD-style test-log service.
...
// Service identity
// ———————————————————————————————————————————————————————————————
const SERVICE_SLUG = "test-log";
const SERVICE_VERSION = 1;
const LOG_FILE = path.resolve(process.cwd(), "test-log-startup-error.log");
...
(async () => {
  try {
    ...
    const { app } = await createApp({
      slug: SERVICE_SLUG,
      version: SERVICE_VERSION,
      envLabel, // <-- this is what test-log/app.ts expects
      envDto: primary,
