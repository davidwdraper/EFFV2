// backend/services/user-auth/src/controllers/user-auth.update.controller/user-auth.update.controller.ts
/**
 * Docs:
 ...
 *
 * Purpose:
 * - Orchestrate PATCH /api/user-auth/v1/:dtoType/update/:id
 * - Thin controller: choose per-dtoType pipeline; pipeline defines handler order.
 *
 * Invariants:
 * - Canonical path param is "id" (legacy :userAuthId normalized to ctx["id"]).
...
import * as UserAuthUpdatePipeline from "./pipelines/update.handlerPipeline";
...
export class UserAuthUpdateController extends ControllerJsonBase {
  ...
  public async patch(req: Request, res: Response): Promise<void> {
    ...
    // Normalize param to canonical "id" (stop userAuthId drift)
    const idParam =
      (req.params as any)?.id ?? (req.params as any)?.userAuthId ?? null;
    ctx.set("id", idParam);
    ...
    switch (dtoType) {
      case "user-auth": {
        ...
        const steps = UserAuthUpdatePipeline.getSteps(ctx, this);
