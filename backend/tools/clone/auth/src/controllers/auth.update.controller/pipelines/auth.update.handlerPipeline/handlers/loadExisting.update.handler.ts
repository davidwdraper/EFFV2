// backend/services/auth/src/controllers/auth.update.controller/handlers/loadExisting.update.handler.ts
/**
 * Docs:
 ...
 *
 * Purpose:
 * - Build DbReader<AuthDto> and load existing doc by canonical ctx["id"].
 * - Returns a **DtoBag** (0..1) as ctx["existingBag"] (does NOT overwrite ctx["bag"]).
 *
 * Inputs (ctx):
 * - "id": string (required; controller sets from :id or :authId)
 * - "update.dtoCtor": DTO class (required)
 *
 * Outputs (ctx):
 * - "existingBag": DtoBag<AuthDto>  (size 0 or 1)
 * - "dbReader": DbReader<AuthDto>
 */
  ...
  protected async execute(): Promise<void> {
    ...
    if (!id) {
      ...
      this.ctx.set("error", {
        ...
        hint: "PATCH /api/auth/v1/<id> with JSON body of fields to update.",
      });
      this.log.debug(
