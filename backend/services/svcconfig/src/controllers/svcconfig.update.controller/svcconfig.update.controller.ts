// backend/services/svcconfig/src/controllers/svcconfig.update.controller/svcconfig.update.controller.ts
/**
 * Docs:
 ...
 *
 * Invariants:
 * - Canonical path param is "id" (legacy :svcconfigId normalized to ctx["id"]).
...
export class SvcconfigUpdateController extends ControllerJsonBase {
  ...
  public async patch(req: Request, res: Response): Promise<void> {
    ...
    const idParam =
      (req.params as any)?.id ?? (req.params as any)?.svcconfigId ?? null;
    ctx.set("id", idParam);
    ...
    switch (dtoType) {
      case "svcconfig": {
        this.seedHydrator(ctx, "svcconfig", { validate: true });
        const steps = SvcconfigUpdatePipeline.getSteps(ctx, this);
